...
감점으로 처리하지 않고 제외하는 이유는, 계단을 이용할 수 없는 사용자에게
계단이 있는 경로는 "불편"이 아니라 "통행 불가"이기 때문이다. 감점만 하면
