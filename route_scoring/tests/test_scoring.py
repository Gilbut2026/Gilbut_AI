...
def test_보행_가능_시간을_크게_초과하면_제외한다():
    ...
    request["candidates"].append(make_candidate("too-far", 4, 1500, 2000, 0, signals()))
...
def test_역_내부와_외부_구간이_섞여도_외부만_계산한다():
    ...
    assert find(score_routes(request)["results"], "mixed")["score"] == find(score_routes(only_external)["results"], "mixed")["score"]
...
def test_점수_상세를_함께_반환한다():
    ...
    assert set(breakdown) == {
        "walkTimePenalty", "walkDistancePenalty", "obstaclePenalty",
        "transferPenalty", "weatherPenalty",
    }
...
def test_육교만_있으면_휠체어_경로를_제외하지_않는다():
    ...
    assert find(score_routes(request)["results"], "overpass")["status"] == "SCORED"


def test_계단이_있으면_휠체어_경로를_제외한다():
    request = make_request(assistiveDevice="WHEELCHAIR", stairAbility="UNAVAILABLE")
    request["candidates"] = [make_candidate("stair", 1, 600, 700, 0, signals(stair=1))]
    assert find(score_routes(request)["results"], "stair")["filterCodes"] == ["WHEELCHAIR_WITH_EXTERNAL_STAIR"]
...
def test_PRESENT인데_count가_0이면_최소_1개로_본다():
    ...
    assert find(score_routes(request)["results"], "mismatch")["status"] == "FILTERED"
